...
bool AMDRyzenCPUPMUserClient::initWithTask(task_t owningTask,
    ...
    token = securityToken;
    // Retain our own reference to the owning task. The task_t
...
// Balance the task_reference() taken in initWithTask(). free()
...
bool AMDRyzenCPUPMUserClient::hasPrivilege(uint32_t selector){
    ...
    // Re-validate root privilege on every call. This adds a small
...
IOReturn AMDRyzenCPUPMUserClient::externalMethod(uint32_t selector, IOExternalMethodArguments *arguments,
    ...
    // OSCompareAndSwap makes the check-and-set atomic.
    ...
    //
    // Centralized pre-switch validation gate.
    //
    ...
    switch (selector) {
        ...
        case 0: {
            ...
            // Reject undersized output buffer to prevent uninitialized kernel memory leak
            if (maxLen < requiredSize) return kIOReturnBadArgument;
            // Report only what actually fits the caller's buffer.
        ...
        case 4: {
            ...
            // Cap at CPUInfo::MaxCpus — effFreq_perCore is sized [MaxCpus];
        ...
        case 10: {
            ...
            // Critical section against concurrent UserClient calls.
        ...
        case 100: {
            ...
            // Zero the packet first. The trap copies out the whole
        ...
        case 28: {
            ...
            // Never emit an uninitialized element. gpuTemperatures[]
            // is zero-initialized and refreshed by the provider's command-gate
            // Timer, so a plain cached copy is always fully defined
        ...
        case 29: {
            ...
            // See case 28 — cached, always-initialized copy.
            for (uint32_t i = 0; i < copyCount; i++) {
        ...
        //Try load SMC driver
        case 90: {
            // The probe itself stays unprivileged so the read-only
        ...
        case 94: {
            ...
            if ((snap94 % 4) == 0) {
                // Refresh the estimator's INPUTS in the same critical
            ...
            uint32_t copyCount = (maxLen / sizeof(uint64_t) < numFans) ? (maxLen / sizeof(uint64_t)) : numFans;
            for (uint32_t i = 0; i < copyCount; i++) {
                // Bit 1 carries per-fan tachometer validity.
                //
        ...
        case 95: {
            ...
            // Command-gated workloop (no live SMN/PCI read here).
            ...
            // Floor a duty that would be commanded to a STOPPED rotor.
            //
            // The active-PWM floor (kCURVE_MIN_ACTIVE_PWM) lives only in
        ...
        case 98: {
            ...
            // A "read" still writes the register index into the
        ...
        // Process-name authorization was removed.
        ...
        // The timer cache, never a live SMU read on a user thread.
        case 44: {
            if(!provider)
                return kIOReturnNoDevice;
        ...
        // From the timer cache, never a live SMU read on a user thread.
        case 47: {
            if(!provider)
                return kIOReturnNoDevice;
        ...
        // Get OC capability report: [0] = 1 when the kext accepts OC-mode
        ...
        // Written). Cache only — no SMU traffic.
        case 52: {
            if(!provider)
                return kIOReturnNoDevice;
        ...
        // Mailbox health diagnostics (privileged): runs the provider's
        ...
        case 58: {
            ...
            // Report only what fits the caller's buffer.
        ...
        // Get PM-table info (read-only): [0] = table version word
        ...
        // Traffic; unsupported on pre-1.30 kexts.
        case 56: {
            if(!provider)
                return kIOReturnNoDevice;
        ...
        // PM-table raw access: op 1 = read a chunk of the snapshot
        ...
        case 57: {
            ...
            uint32_t maxLen = arguments->structureOutputSize;
            // Reject undersized output buffers; // Report only what actually fits.
