...
/// OLD PATH ("/Applications/RyzenStatus Utils.app"), so right after updating the
/// app is running from a bundle still named "RyzenStatus Utils.app". We rename
/// that bundle to "RyzenStatus.app" through a detached helper that runs only after
...
///
/// When we are already running as "RyzenStatus.app", we instead retire a stray
...
enum BundleMigration {
    private static let oldName = "RyzenStatus Utils.app"
    private static let newName = "RyzenStatus.app"
    ...
    private static func renameSelfAndRelaunch(from old: URL) -> Bool {
        ...
        let scriptURL = FileManager.default.temporaryDirectory
            .appendingPathComponent("ryzenstatus-rename-\(pid)-\(UUID().uuidString).sh")
        do {
