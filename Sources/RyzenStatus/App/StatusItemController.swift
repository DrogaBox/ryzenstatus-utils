...
final class StatusItemController {
    ...
    private func syncMonitorMode() {
        ...
        let interval = Defaults.sanitizedMonitorInterval(defaults.double(forKey: DefaultsKey.monitorInterval))
    ...
    @objc private func metricClicked(_ sender: NSStatusBarButton) {
        if NSApp.currentEvent?.type == .rightMouseUp {
            if let item = metricStatusItems.values.first(where: { $0.button == sender }) {
                onRightClick?(item)
            } else {
                onRightClick?(statusItem)
            }
