...
enum RecordingSharingSupport {
    ...
    /// endpoint (same override the screenshot sharing uses), and only in the
    /// Developer build. The shipped app must never send recordings to a host
    /// we do not control, so there is deliberately NO production fallback.
