...
final class WhatsAppDownloadOrganizer: ObservableObject {
    ...
    private let queue = DispatchQueue(label: "com.ryzenstatus.whatsapp-organizer",
                                      qos: .utility)
        ...
        for source in files where settings.categories.contains(source.category) {
            ...
            do {
                ...
                if let duplicateIndex = validDuplicateIndex {
                    ...
                    case .trashNew:
                        _ = try trash(source.url)
                        duplicates += 1
                        continue
                    case .replaceExisting:
                        _ = try replaceVerified(source: source.url,
                                                existing: existing,
                                                digest: digest)
                ...
                _ = try moveVerified(source: source.url,
                                     destination: destination,
                                     digest: digest)
    ...
    private static func moveVerified(source: URL,
        ...
        let temporary = destination.deletingLastPathComponent().appendingPathComponent(
            ".ryzenstatus-\(UUID().uuidString).partial")
        do {
    ...
    private static func replaceVerified(source: URL,
        ...
        let staging = existing.deletingLastPathComponent().appendingPathComponent(
            ".ryzenstatus-replaced-\(UUID().uuidString).partial")
