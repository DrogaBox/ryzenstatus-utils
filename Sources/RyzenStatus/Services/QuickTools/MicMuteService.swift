...
final class MicMuteService: ObservableObject {
    ...
    private var installedListeners: [(selector: AudioObjectPropertySelector, block: AudioObjectPropertyListenerBlock)] = []
    ...
    private let halQueue = DispatchQueue(label: "com.ryzenstatus.utils.micmute.hal", qos: .userInitiated)
    ...
    private func installListeners() {
        let installedSelectors = installedListeners.map(\.selector)
        for selector in Self.watchedSelectors where !installedSelectors.contains(selector) {
            var address = AudioObjectPropertyAddress(mSelector: selector,
                                                     mScope: kAudioObjectPropertyScopeGlobal,
                                                     mElement: kAudioObjectPropertyElementMain)
            ...
            if status == noErr {
                installedListeners.append((selector, block))
            }
        }
    }

    private func removeListeners() {
        for (selector, block) in installedListeners {
            var address = AudioObjectPropertyAddress(mSelector: selector,
                                                     mScope: kAudioObjectPropertyScopeGlobal,
                                                     mElement: kAudioObjectPropertyElementMain)
