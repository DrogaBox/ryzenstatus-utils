...
final class ClipboardHistoryService: ObservableObject {
    ...
    private static let persistQueue = DispatchQueue(label: "com.ryzenstatus.utils.clipboard-persist",
                                                    qos: .utility)
    ...
    func copy(_ selectedEntries: [ClipboardHistoryEntry], completion: ((Bool) -> Void)? = nil) {
        guard !selectedEntries.isEmpty else { completion?(false); return }
    ...
    /// clearContents() already destroyed it. Handed to the lane so a read from
    ...
    private func start() {
        ...
        self.timer = timer
        isRunning = true
        // The ignore list only tracks the frontmost app while the
    ...
    private func stop() {
        ...
        timer = nil
        isRunning = false
        // Release the activation observer with the capture loop.
    ...
    private func captureIfChanged() {
        ...
        GeneralPasteboardAccess.shared.async { [weak self] in
            ...
            DispatchQueue.main.async {
                guard let self, self.captureGeneration == generation else { return }
                self.captureInFlight = false
                // A copy noticed while an ignored app was frontmost
    ...
    private static func readPasteboard(includeImagesFiles: Bool) -> CapturedContent? {
        let pasteboard = NSPasteboard.general
        // Skip concealed/password items from password managers (issue #415)
        if isConcealedOrPasswordPasteboard(pasteboard) { return nil }
    ...
    private static func isConcealedOrPasswordPasteboard(_ pasteboard: NSPasteboard) -> Bool {
        guard let types = pasteboard.types else { return false }
        let excludedTypes: Set<String> = [
            "org.nspasteboard.ConcealedType",   // standard concealed pasteboard protocol
            // Standard reverse-DNS UTIs used by Safari AutoFill, Apple Keychain, KeePassXC
            "org.nspasteboard.TransientType",
            "org.nspasteboard.AutoGeneratedType",
            "com.agilebits.onepassword",          // 1Password
            "org.bitwarden.clipboard",            // Bitwarden
            "com.bitwarden.bitwarden",            // Bitwarden (alternate UTI)
            "com.typeit4me.clipping",             // TypeIt4Me
            "TransientType",                      // generic transient / ephemeral
            "AutoGeneratedType"                   // auto-generated sensitive content
        ]
        return types.contains { excludedTypes.contains($0.rawValue) }
    }
    ...
    private func persist() {
        ...
        Self.persistQueue.async { [weak self] in
            ...
            // This JSON is the full private history (password-adjacent
...
enum ClipboardImageStore {
    ...
    static func store(_ data: Data) -> String? {
        ...
        // Copied screenshots are private content — same 0600/0700
