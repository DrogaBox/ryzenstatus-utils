...
enum HomebrewCommandBuilder {
    ...
    static let installerCommand = #"/bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install.sh)""#
    ...
    static func search(brewPath: String, kind: HomebrewPackageKind, query: String) -> HomebrewCommand {
        ...
        let sanitized = token.drop { $0 == "-" }
        return HomebrewCommand(executable: brewPath, arguments: ["search", flag, String(sanitized)])
    }
    ...
    private static let untrustedTapTrimSet = CharacterSet(charactersIn: ".")
    ...
    static func untrustedTapName(fromOutput output: String) -> String? {
        ...
        let name = String(output[range])
            ...
            .trimmingCharacters(in: untrustedTapTrimSet)
...
enum HomebrewProgressParser {
    ...
    private static func isMostlyProgressSymbols(_ value: String) -> Bool {
        let allowed = progressSymbolsSet
