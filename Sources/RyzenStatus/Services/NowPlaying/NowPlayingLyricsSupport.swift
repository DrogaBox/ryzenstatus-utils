...
enum NowPlayingLyricsText {
    // Cache static regular expressions for performance.
    // Hoisting these prevents expensive re-allocation and re-compilation
    // of patterns during parsing of long lyric text bodies.
    private static let lrcRegex = try? NSRegularExpression(pattern: #"\[(\d{1,2}):(\d{2})(?:\.(\d{1,3}))?\]"#)
    private static let lrcOffsetRegex = try? NSRegularExpression(pattern: #"\[offset:\s*([+-]?\d+)\]"#,
    ...
    static func parseLRC(_ raw: String) -> [NowPlayingLyricsLine]? {
        ...
        let offsetSeconds: TimeInterval = {
            guard let offsetRegex = lrcOffsetRegex,
