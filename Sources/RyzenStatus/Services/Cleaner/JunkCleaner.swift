...
final class JunkCleaner: ObservableObject {
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            let categories: [(CleanerSupport.Category, () -> [Item])] = [
                (.leftovers, {
                    ...
                    claimed = Set(found.map { $0.url.standardizedFileURL.path })
                    return found
                }),
    ...
    private static func installedBundleIDs() -> Set<String> {
        ...
        func remember(app url: URL) {
            ...
            let loginItems = url.appendingPathComponent("Contents/Library/LoginItems", isDirectory: true)
            collect(at: loginItems, depth: 1)
        }

        func collect(at url: URL, depth: Int) {
            guard depth > 0 else { return }
            guard !UninstallerSupport.isSymbolicLink(url) else { return }
        ...
        for root in roots {
            collect(at: URL(fileURLWithPath: root, isDirectory: true), depth: 3)
        }
