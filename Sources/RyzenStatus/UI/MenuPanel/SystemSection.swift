...
struct SystemSection: View {
    ...
    private func powerCell(label: String, watts: Double) -> some View {
        VStack(spacing: 3) {
            Text(label)
                .font(.system(size: 10))
                .foregroundStyle(.secondary)
    ...
    private func batteryUsageRow(editing: Bool) -> some View {
        if let charge = monitor.snapshot.power?.chargePercent {
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 8) {
                    let isAC = monitor.snapshot.power?.externalConnected ?? false
                    let isCharging = monitor.snapshot.power?.isCharging ?? false
                    Image(systemName: isAC && !isCharging ? "macpro.gen3.fill" : (isCharging ? "bolt.fill" : "battery.100"))
                        .font(.system(size: 9))
                        .foregroundStyle(.secondary)
                        .frame(width: 10)
                ...
                if let gpuUsed = monitor.snapshot.gpuMemoryUsed, let gpuTotal = monitor.snapshot.gpuMemoryTotal {
                    HStack(spacing: 8) {
                        ...
                        Text("GPU VRAM")
                            ...
                            .foregroundStyle(.secondary)
                        Spacer()
                        Text("\(formatMemory(gpuUsed)) / \(formatMemory(gpuTotal))")
                            .font(.system(size: 11, weight: .medium))
                            .monospacedDigit()
                    }
                }
