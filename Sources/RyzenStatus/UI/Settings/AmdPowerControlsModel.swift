...
final class AmdPowerControlsModel: ObservableObject {
    ...
    // Rolling history for the AMD panel sparklines — 60 samples ≈ 3 min
    ...
    // SMU boost telemetry (selector 43) — refreshed inside syncFromKext's
    ...
    // ProcessorParameters (0x6F) + cHTC limit cache — refreshed inside
    // SyncFromKext's detached kext batch alongside the boost telemetry.
    ...
    // SMU firmware version (0x02, one-shot) + active PBO scalar (0x6C,
    ...
    // OC capability + mode cache (selectors 49/50). `ocModeCode` mirrors
    ...
    // Frequency-override support flag (selector 49 [3], promoted from
    ...
    // PM-table plumbing snapshot info (selector 56). All diagnostic —
    ...
    // Decoded per-core/package telemetry (nil until a known-version
    ...
    // Per-core rolling windows for the panel sparklines — [slot → history],
    ...
    // On-demand mailbox health report (selector 58). Never polled in the
    ...
    func syncFromKext() async {
        ...
        let (kernelAnswered, cpb, cppcState, ppm, lpm, boost, procParams, chtcLimit,
            ...
            // Cached boost telemetry — the kext call never touches the SMU,
            // it only reads the timer-populated cache.
            let boost = kernelAnswered ? ProcessorModel.shared.getBoostTelemetry() : nil
            // One-shot 0x6F bitfield + cHTC cache, same off-the-SMU policy.
            ...
            // One-shot 0x02 version + 0x6C active scalar readbacks.
            ...
            // OC capability + mode cache (no SMU traffic — cache only).
            let ocCap = kernelAnswered ? ProcessorModel.shared.getOcCapability() : nil
            // Frequency-override cache (cache only, no SMU traffic).
            let ocFreq = kernelAnswered ? ProcessorModel.shared.getOcFreqCache() : nil
            // PM-table info (cache only, no SMU traffic).
        ...
        // Publish the boost telemetry snapshot (or clear it when the kext
        ...
        // Publish the ProcessorParameters bitfield + cHTC limit cache.
        if let procParams {
        ...
        // Publish the SMU version + active scalar readbacks.
        if let smuVersion {
        ...
        // Publish the OC capability + mode cache.
        if let ocCap {
        ...
        // Publish the frequency-override cache. `ocFreqSupported` reads
        ...
        // Publish the PM-table plumbing info.
        if let pmInfo {
        ...
        // Consume ProcessorModel's single-owner decode instead of
        ...
        // The diagnostics report is deliberately NOT refreshed here —
        // the kext adds real SMU mailbox traffic to answer, so it only
        // updates when the user asks (runMailboxDiagnostics).
        // Feed the per-core sparkline windows from the same decode.
    ...
    /// Run the mailbox health probes on demand (selector 58, root or
    ...
    /// One-shot text bundle for bug reports — versions, chip identity,
