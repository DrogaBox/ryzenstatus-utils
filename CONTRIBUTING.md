...
to create a free, self signed identity called `RyzenStatus Utils Signing` in a
...
| Folder | Role |
|---|---|
| `Sources/RyzenStatus/App` | App lifecycle and the menu bar status item |
| `Sources/RyzenStatus/Core` | Localization, permissions, UserDefaults keys |
| `Sources/RyzenStatus/Services` | All behavior, like energy, monitor, scroll and switcher |
| `Sources/RyzenStatus/UI` | SwiftUI views only, no business logic |
| `Sources/RyzenStatus/Support` | `--selftest` and `--sensors` diagnostics |
